  ...
  <UCard class="max-w-7xl mx-auto  bg-gray-50 dark:bg-gray-900 ">
    ...
    <UForm :state="modelValue" class="flex flex-col space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <UButton v-if="sponsersAvilabel" :disabled="disabledFields?.sponsors" @click="AddSponser()" color="primary" variant="soft" icon="i-heroicons-plus" size="sm">إضافة راعي</UButton>
        <input ref="SponsorInput" type="file" name="sponsor" class="hidden" :disabled="disabledFields?.sponsors" @input="onSponsorsChange($event)" accept=".png,.jpg,.jpeg" />
      </div>
        ...
        <div class="flex gap-4 overflow-x-auto pb-4">
          <div v-for="(image, index) of SponsorsUrl" :key="index" class="relative group flex-shrink-0">
            <div class="w-24 h-24 md:w-32 md:h-32 rounded-lg overflow-hidden">
              ...
              <button :disabled="disabledFields?.sponsors" @click="removeSponsors(index)" class="absolute inset-0 bg-red-500/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 disabled:cursor-not-allowed">
...
const props = defineProps<{ modelValue: any; errors?: Record<string, string | undefined>; onFieldBlur?: (field: string) => void; disabledFields?: Record<string, boolean>; initialLogoUrl?: string }>();
...
const openLogoInput = () => { if (props.disabledFields?.logo) return; fileInput.value?.click(); }
const removeLogo = () => { if (props.disabledFields?.logo) return; logoImageUrl.value = ""; props.modelValue.logo = undefined; }
...
watch(() => props.disabledFields?.sponsors, (isDisabled) => {
  if (isDisabled) sponsersAvilabel.value = !!props.modelValue.sponsors?.length;
});
