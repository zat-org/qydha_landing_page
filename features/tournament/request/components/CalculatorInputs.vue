...
const TablesCountOptions = [{ label: "2 طاولة", value: 2 },{ label: "4 طاولات", value: 4 },{ label: "8 طاولات", value: 8 },{ label: "10 طاولات", value: 10 },{ label: "عدد اخر", value: "custom" }]
const getInitialTeamsOption = () => { if (teamsCount.value) { const option = TeamsCountOptions.find(opt => opt.value === teamsCount.value); return option ? option.value : 'custom' } return 'custom' }
const getInitialTablesOption = () => { if (tablesCount.value) { const option = TablesCountOptions.find(opt => opt.value === tablesCount.value); return option ? option.value : 'custom' } return 'custom' }
...
watch(teamsCountOption, (newValue) => { if (newValue !== 'custom' && typeof newValue === 'number') teamsCount.value = newValue })
watch(tablesCountOption, (newValue) => { if (newValue !== 'custom' && typeof newValue === 'number') tablesCount.value = newValue })
watch(teamsCount, (newValue) => { const option = TeamsCountOptions.find(opt => opt.value === newValue); if (option && teamsCountOption.value !== option.value) teamsCountOption.value = option.value; else if (!option && teamsCountOption.value !== 'custom') teamsCountOption.value = 'custom' })
watch(tablesCount, (newValue) => { const option = TablesCountOptions.find(opt => opt.value === newValue); if (option && tablesCountOption.value !== option.value) tablesCountOption.value = option.value; else if (!option && tablesCountOption.value !== 'custom') tablesCountOption.value = 'custom' })
</script>
