    ...
    >
      ...
      <div>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <UButton
            :to="`/tournament/${id}/table`"
            label="الطاولات"
            icon="i-mdi-table"
            size="lg"
            variant="soft"
            color="neutral"
            class="min-h-12 w-full justify-center sm:justify-start"
            block
          />
          <UButton
            :to="`/tournament/${id}/refree`"
            label="الحكام"
            icon="i-mdi-gavel"
            size="lg"
            variant="soft"
          ...
          <div class="-mx-1 overflow-x-auto scroll-smooth overscroll-x-contain px-1 pb-2 touch-pan-x" dir="rtl">
            <div class="flex min-w-max items-start gap-0 py-1">
              <template v-for="(phase, idx) in phaseOrder" :key="phase">
                ...
                <div
                  v-if="idx < phaseOrder.length - 1"
                  class="mt-[26px] h-0.5 w-5 shrink-0 rounded-full transition-colors sm:w-8"
                  :class="idx < currentPhaseIndex ? 'bg-primary' : 'bg-gray-200 dark:bg-gray-700'"
                  aria-hidden="true"
                />
              </template>
            </div>
      ...
      <div aria-label="المرحلة الحالية">
        ...
        >
          ...
          <div class="space-y-4">
            <h3
              v-if="phaseBehavior.content.heading"
              class="text-base font-semibold text-gray-800 dark:text-gray-100"
            >
              {{ phaseBehavior.content.heading }}
            </h3>
            <p
              v-if="phaseBehavior.content.description"
              class="text-sm leading-relaxed text-gray-600 dark:text-gray-300"
            >
              {{ phaseBehavior.content.description }}
            </p>
            <UAlert
              v-if="phaseBehavior.content.alert"
              :color="phaseBehavior.content.alert.color"
              variant="subtle"
              :title="phaseBehavior.content.alert.title"
              :description="phaseBehavior.content.alert.description"
            />
            <div v-if="visiblePhaseActions.length" class="flex flex-col flex-wrap gap-3 sm:flex-row">
              <UButton
                v-for="action in visiblePhaseActions"
                :key="`${phaseBehavior.panel}-${action.label}`"
                :to="action.to ? `/tournament/${id}${action.to}` : undefined"
                :label="action.label"
                :icon="action.icon"
                ...
                :loading="action.event === 'reset-final-group-matches' && isResetFinalGroupMatchesPending"
                :disabled="action.event === 'reset-final-group-matches' && isResetFinalGroupMatchesPending"
                block
...
const handlePhaseAction = (event?: TournamentPhaseAction["event"]) => {
  if (!event) return;
  if (event === "organize-tournament") emit("organize-tournament");
  if (event === "open-start-confirm") emit("open-start-confirm");
  if (event === "reset-final-group-matches") emit("reset-final-group-matches");
};
</script>
