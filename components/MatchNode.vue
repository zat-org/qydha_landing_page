<template>
  <div>
    ...
    <div class="flex flex-col w-[300px] h-[75px] text-xs font-semibold bg-white p-1  rounded">
      <div
        :class="{ 'bg-green-300': data.winner == 'Us', 'bg-red-300': data.state == 'Ended' && (data.winner != 'Us' || !data.winner) }"
        ...
        <p v-if="data.usTeam" class="grow text-center"
          :class="{ 'line-through': data.state == 'Ended' && !data.qydhaGameId && (data.winner != 'Us' || !data.winner) }">
          {{ data.usTeam.name }}
        </p>
        <p class="grow text-center" v-else-if="!data.usTeam && data.state !== 'Ended'">لم يحدد بعد</p>
        <p class="grow text-center" v-else-if="!data.usTeam && data.state === 'Ended'">انسحب كلا الفريقين</p>
      </div>

      <div
        :class="{ 'bg-green-300': data.winner == 'Them', 'bg-red-300': data.state == 'Ended' && (data.winner != 'Them' || !data.winner) }"
        ...
        <p v-if="data.themTeam" class="grow text-center"
          :class="{ 'line-through': data.state == 'Ended' && !data.qydhaGameId && (data.winner != 'Them' || !data.winner) }">
          {{ data.themTeam.name }}
        </p>
        <p class="grow text-center" v-else-if="!data.themTeam && data.state != 'Ended'">لم يحدد بعد</p>
        <p class="grow text-center" v-else-if="!data.themTeam && data.state == 'Ended'">انسحب كلا الفريقين</p>
      </div>
      <div class="flex justify-between text-center h-7">
        ...
        <div class="flex items-center bg-gray-300 rounded  rounded-t-none border boreder-gray-500 px-1  gap-2">
          ...
          <UIcon v-if="data.state == 'Ended'" name="material-symbols:done-all" />
