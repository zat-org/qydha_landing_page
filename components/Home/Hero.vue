<template>
  <section
    id="hero"
    class="flex justify-center items-center min-h-[90vh] relative overflow-hidden bg-white dark:bg-gray-900"
  >
    <!-- Background decoration -->
    <div
      class="absolute inset-0 bg-linear-to-br from-amber-50/50 to-blue-50/50 dark:from-gray-800/50 dark:to-gray-900/50"
    ></div>
    <div
      class="absolute inset-0 bg-center bg-no-repeat opacity-10 dark:opacity-5"
    ></div>

    <!-- Animated gradient overlay -->
    <div
      class="absolute inset-0 bg-linear-to-r from-transparent via-white/5 to-transparent dark:via-gray-800/5 animate-shimmer bg-size-[200%_100%]"
    ></div>

    <div
      class="container mx-auto px-4 sm:px-4 flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10"
    >
      <div class="w-full lg:w-1/2 space-y-8 text-center lg:text-right">
        <div class="space-y-5">
          <h2
            class="text-4xl sm:text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white leading-loose dark:drop-shadow-lg"
          >
            <span
              class="py-2 bg-linear-to-r from-gray-900 to-gray-700 dark:from-primary-400 dark:to-primary-600 bg-clip-text text-transparent"
            >
              ماتحلى الجلسه
            </span>
            <br />
          ...
          <p
            class="text-lg sm:text-xl text-gray-700 dark:text-gray-300 leading-relaxed max-w-2xl mx-auto lg:mx-0"
          >
            سهلناها عليكم … تطبيق حاسبة بلوت بإنعكاس شاشتين👏
            <br />
            سرعوا اللعب يا شباب حملوا التطبيق وخلونا نشوف لعبكم
          </p>
        </div>

        <div
          class="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center lg:justify-start items-center"
        >
          <UButton
            color="primary"
