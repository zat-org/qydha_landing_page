...
const onSubmit = async () => {
  ...
  if (updatePlayerREQ.status.value == "error" && updatePlayerREQ.error.value) {
    if (updatePlayerREQ.error.value.statusCode == 400) {
      Object.entries(updatePlayerREQ.error.value.data?.errors!).forEach(e => {
        PlayerForm.value?.setErrors([{ path: e[0].toLowerCase(), message: e[1][0] }])
      })
    }
    if (updatePlayerREQ.error.value.statusCode == 404 && updatePlayerREQ.error.value.data?.code == 'EntityNotFound') {
