    ...
    >
      ...
      <div class="flex flex-col gap-3">
        ...
        <UFormField 
          ...
          :label="`قيمة الجائزة`" 
          :name="`TournametPrize[${index}].money`"
        >
          <UInput 
            v-model.number="prize.money" 
            type="number" 
        ...
        <UFormField v-show="prize.isMoney" :label="`العملة`" :name="`TournametPrize[${index}].currency`">
          <CurrencyInput v-model="prize.currency" /> 
        </UFormField>
        <!-- Items Prize Section -->
        <UFormField 
          v-show="prize.isItem"
          :label="`الجوائز العينية`" 
          :name="`TournametPrize[${index}].items`"
        >
          <div class="space-y-2">
            <!-- Add Item Input -->
            ...
            <div v-if="prize.items.length > 0" class="flex flex-wrap gap-1.5">
              <span 
                v-for="(item, itemIndex) in prize.items" 
                :key="itemIndex"
...
const prizeOptions = [
  { label: "مركز الاول فقط", value: 1 },
  { label: "مركز الاول والثاني فقط", value: 2 },
  { label: "مركز الاول والثاني والثالث فقط", value: 3 },
  { label: "مركز الاول والثاني والثالث والرابع فقط", value: 4 }
];
