    ...
    <div class="flex flex-col">
      <div class="flex justify-between items-center px-5">
        <img
          :src="
            tour.logoUrl
              ? tour.logoUrl
              : 'https://th.bing.com/th/id/OIP.PAlqgKSssv6ig-bjh9KEtwHaO8?rs=1&pid=ImgDetMain'
          "
          class="w-[100px]"
        />
        <div class="flex flex-col justify-center ml-auto mr-[10px]">
          ...
          <p>
            التاريخ : {{ new Date(tour.startAt).toDateString() }} -
            {{ new Date(tour.endAt).toDateString() }}
          </p>
      ...
      <div class="flex flex-wrap gap-5 mt-[20px] justify-center">
        ...
        <UButton
          label="ادارة الفرق "
          size="lg"
          :to="'/tournament/' + id + '/team'"
        />
        <UButton
          label="player managment  "
          size="lg"
          :to="'/tournament/' + id + '/player'"
        />
        <UButton
          label="table Mangment"
          size="lg"
    ...
    <template #footer>
      <div class="flex justify-between">
        ...
        <UButton
          ...
          v-if="
            privilege?.toLowerCase() == 'admin' ||
            privilege?.toLowerCase() == 'owner' ||
            permissions.includes('ModifyTournamentData')
          "
        />
      </div>
    </template>
