<template>
  ...
  <UCard :ui="{base:'w-[80%] mx-auto mt-[50px]'}">
    ...
    <UForm :schema="usernameSchema" :state="state" class="space-y-3" v-if="state.formState == FormState.waitUsernameInput" >
      <UFormGroup name="username" label="يوزر قيدها" hint="مطلوب" >
        ...
        type="text"  icon="i-heroicons-at-symbol-20-solid"
      ...
      <div class="flex justify-end">
        <UButton v-if="state.formState === FormState.waitUsernameInput" type="submit"
        @click="onLogInQydha"
    ...
    <UForm :schema="otpSchema" @submit="onConfirm" v-if="state.formState === FormState.waitOtpInput" :state="state" class=" my-2 space-y-3"
     >
     <UFormGroup label="رمز الدخول"  name="otp" hint="مطلوب">

       <UInput dir="ltr" v-model="state.otp" type="text"
