      ...
      <UTabs :items="tabItems" @change="onChangeindex">
        <template #data>
          <UForm :state="state" :schema="schema" ref="poupForm" @submit="onSubmit" class="flex flex-col gap-2">
            ...
            <UFormGroup label="التارجيت" name="actionPath"
              v-if="state.actionType != AssetPopUpActionType.PopUpWithNoAction && state.actionType">
              ...
              <USelect v-model="state.actionPath"
                :options="state.actionType == AssetPopUpActionType.PopUpWithGoToScreen ? screenOptions : tabOptions"
                v-if="state.actionType == AssetPopUpActionType.PopUpWithGoToScreen || state.actionType == AssetPopUpActionType.PopUpWithGoToTab" />
            </UFormGroup>
          ...
          <UForm :state="imageState" :schema="imageSchema" ref="imageForm" @submit="onSubmitImage">
            <div class="flex justify-between items-center ">
              <UFormGroup label="الصورة" name="image">
                <UInput type="file" @change="onChange" accept=".jpg , .png , .jpeg " v-model="imageState.image"> </UInput>
              </UFormGroup>
...
import { AssetPopUpActionType, type IAssetPopUpUpdate } from '~/models/assetPopup';
const tabindex = ref(0)
const onChangeindex = (index: number) => {
  tabindex.value = index
}
const tabItems = [{ label: "data", slot: 'data' }, { label: "image", slot: 'image' }]
...
const notificationActionsArray = Object.values(AssetPopUpActionType).map(action => ({
  value: action,
  label: action,
}));
